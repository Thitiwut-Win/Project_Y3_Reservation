...
async function fetchQR (token, userId, eventId, amount, uuid) {
  ...
  const random2 = nanoid(15);
  const ref1 = `TXN${date}${random1}`;
  const ref2 = crypto.createHash("sha256").update(`${userId}:${eventId}:${process.env.HASH_SECRET}`).digest("hex").slice(0, 20);
  const ref3 = `${process.env.REF_PREFIX}${random2}`;

  const response = await ("https://api-sandbox.partners.scb/partners/sandbox/v1/payment/qrcode/create", {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
			"resourceOwnerId": process.env.SCB_API_KEY,
...
export const createPayment = async (req, res) => {
  try {
    ...
    console.log(QRResponse.status.code, QRResponse.status.description)
